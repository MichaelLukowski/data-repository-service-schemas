...
Each implementation of DRS can choose a persistent identifier to resolve DRS objects persistently if needed. We support Archival Resource Keys `ark` and dataguids `dg` at the moment. See https://n2t.net/e/ark_ids.html or https://dataguids.org/. These are represented first with a schema `ark:` for Archival Resource Keys and `dg.` for dataguids. Secondly there is an identifier that your DRS implementation uses. This is a unique string assigned by a resolution service like https://nt2.net or https://dataguids.org/. Once assigned it is shared with other resolvers so that they can resolve the prefixes as well.
...
When making DRS requests if a 404 is encountered we are able to use the DRS prefix to resolve the identifiers. We can use a resolution service like http://n2t.net/[n2t], http://identifiers.org/[identifiers.org], or https://dataguids.org/[dataguids] (for `dg.`) to resolve our prefixes. This will give us a HTTP 302 response with the redirect to the new URL associated with the DRS implementation. This new URL can then be cached to be used for subsequent DRS requests.
